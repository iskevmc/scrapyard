...
class RepoUtils {
    ...
    static IsTeamFoundationGitRepo(url) {
        if (!url) {
            return false;
        }
    ...
    static IsTeamFoundationServicesRepo(url) {
    ...
    static IsTeamFoundationServicesAzureRepo(respositoryUrl) {
        try {
            ...
            const purl = url.parse(respositoryUrl);
            if (purl.hostname.toLowerCase().indexOf("azure.com") >= 0) {
                return true;
            }
        }
        catch (err) {
            logger_1.Logger.LogDebug("Could not parse repository url: " + respositoryUrl);
        }
        return false;
    }
    static IsTeamFoundationServicesV3SshRepo(respositoryUrl) {
        return RepoUtils.sshV3.test(respositoryUrl.toLowerCase());
    }
    static ConvertSshV3ToUrl(respositoryUrl) {
        const scheme = "https://";
        ...
        logger_1.Logger.LogDebug("Could not parse as v3 repository url: " + respositoryUrl);
        return undefined;
    }
    //This is the "catch all" method.  A repository hosted on TFS is defined by not being on "visualstudio.com"
    static IsTeamFoundationServerRepo(url) {
...
RepoUtils.sshV3 = new RegExp("git@(?:vs-)?ssh\.(.+):v3\/(.+)\/(.+)\/(.+)");
exports.RepoUtils = RepoUtils;
