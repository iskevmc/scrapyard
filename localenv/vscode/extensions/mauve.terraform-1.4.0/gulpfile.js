...
gulp.task('generate-hcl-container', (done) => {
  var docker = spawn(
      'docker',
      [
        'build', '-t', 'gopher-hcl-gopherjs', '-f',
        'hcl-hil/gopherjs.Dockerfile', 'hcl-hil'
      ],
      {stdio: 'inherit'});
  ...
  docker
      .on('error',
          (err) => {
            ...
            log.error(`${chalk.red('ERROR')}: Cannot launch "docker": ${
                chalk.bold(err)}.`);
            log.error(` ${
                chalk.yellow(
                    'INFO')}: Docker is required for building, you can install it from https://www.docker.com`);
...
gulp.task(
    'generate-transpiled.js', gulp.series('generate-hcl-container', (done) => {
      var docker = spawn(
          'docker', ['run', '--rm', 'gopher-hcl-gopherjs'],
      ...
      docker.on('close', (code) => {
        if (code !== 0) {
              ...
              `docker run gopher-hcl-gopherjs failed with code ${code}`));
        } else {
          done();
        }
      });
...
gulp.task(
    'generate-closure-container',
    gulp.series('generate-transpiled.js', (done) => {
      var docker = spawn(
          'docker',
          [
            'build', '-t', 'gopher-hcl-closure-compiler', '-f',
            'hcl-hil/closure.Dockerfile', 'hcl-hil'
          ],
          {stdio: 'inherit'});
...
gulp.task(
    'generate-hcl-hil.js',
    gulp.series(
        'create-output-directory', 'generate-closure-container', (done) => {
          var docker = spawn(
              'docker', ['run', '--rm', 'gopher-hcl-closure-compiler'],
              {stdio: ['ignore', 'pipe', 'inherit']});

          var stream =
              fs.createWriteStream('out/src/hcl-hil.js', {flags: 'w+'});
...
gulp.task(
    ...
    () => gulp.src('src/data/*.json')
              .pipe(using(
                  {prefix: 'Bundling auto-completion data', filesize: true}))
...
gulp.task(
    ...
    () => project.src()
              ...
              .pipe(sourcemaps.mapSources((sourcePath, file) => {
                let relativeLocation = path.join(
                    path.relative(
                        path.join('out', path.dirname(file.relative)), '.'),
                    'src/');
                let relativeLocationToFile =
                    path.join(relativeLocation, sourcePath);
                return relativeLocationToFile;
              }))
              .pipe(sourcemaps.write('.', {includeContent: false}))
              .pipe(gulp.dest('out')));

// generate telemetry file (depend on copy-html-templates so that directory is
// created)
gulp.task(
    'generate-constants-keyfile',
    gulp.series('create-output-directory', (done) => {
      let contents = {APPINSIGHTS_KEY: process.env.APPINSIGHTS_KEY};

      if (!contents.APPINSIGHTS_KEY) {
        // Do not require the key for now as it actually doesnt exist anymore,
        // but leaving this code in so that we can resurrect it soonishly. if
        // ((process.env.CI || helpers.requireAppInsightsKey) &&
        // process.env.TRAVIS_PULL_REQUEST !== "true") {
        //    log.error(`${chalk.red('ERROR')}: AppInsights Key missing in CI
        //    build`); done(new Error("AppInsights Key missing in CI build, set
        //    APPINSIGHTS_KEY environment variable"));
        // } else {
        log.warn(` ${
            chalk.yellow(
                'WARN')}: AppInsights Key not bundled, this build will NOT emit metrics.`);
        // }
      } else {
        log.info(` ${
            chalk.green(
                'INFO')}: AppInsights Key bundled, this build will emit metrics`);
      }
...
function test() {
  return gulp.src(['test/**/*.unit.test.ts'], {read: false})
      .pipe(mocha({reporter: 'spec', ui: 'tdd', require: 'ts-node/register'}));
}

gulp.task(test);

gulp.task('test-no-fail', () => {
  return test().on('error', (err) => {
...
gulp.task('generate-release-notes', function generateReleaseNotes(done) {
  fs.readFile('./CHANGELOG.md', (err, data) => {
    ...
    fs.writeFile(
        'out/LAST_RELEASE.md',
        lines.slice(firstHeaderLine, secondHeaderLine).join('\n'), (err) => {
          if (err) {
            log.error(
                `${chalk.red('ERROR')}: could not write out/LAST_RELEASE.md: ${
                    err}`);
            done(err);
          } else {
            done();
