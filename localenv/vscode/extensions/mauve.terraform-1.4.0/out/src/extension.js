...
const toggleLanguageServer_1 = require("./commands/toggleLanguageServer");
const installLanguageServer_1 = require("./commands/installLanguageServer");
...
function activate(ctx) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (configuration_1.getConfiguration().languageServer.enabled && configuration_1.getConfiguration().indexing.enabled) {
        ...
        const languageServerClient = new languageclient_1.ExperimentalLanguageClient(ctx);
        ctx.subscriptions.push(new toggleLanguageServer_1.ToggleLanguageServerCommand(ctx));
        ctx.subscriptions.push(new installLanguageServer_1.InstallLanguageServerCommand(ctx));
        if (configuration_1.getConfiguration().languageServer.enabled) {
            yield languageServerClient.start();
        }
        else {
            informUserAboutLspIfTf12();
        }
        ctx.subscriptions.push(new lint_1.LintCommand(ctx));
        if (configuration_1.getConfiguration().indexing.enabled) {
            ctx.subscriptions.push(new plan_1.PlanCommand(runner, indexAdapter, ctx), new commands_1.IndexCommand(indexAdapter, ctx), new validate_1.ValidateCommand(indexAdapter, runner, ctx), new showreferences_1.ShowReferencesCommand(indexAdapter, ctx), new navigatetosection_1.NavigateToSectionCommand(indexAdapter, ctx), new preview_1.PreviewGraphCommand(indexAdapter, runner, ctx), new reindex_1.ReindexCommand(indexAdapter, watcher, ctx));
            // providers
            vscode.languages.registerCompletionItemProvider(documentSelector, new completion_provider_1.CompletionProvider(indexAdapter), '.', '"', '{', '(', '['),
                vscode.languages.registerDefinitionProvider(documentSelector, new definition_1.DefinitionProvider(indexAdapter)),
                vscode.languages.registerDocumentSymbolProvider(documentSelector, new providers_1.DocumentSymbolProvider(indexAdapter)),
                vscode.languages.registerWorkspaceSymbolProvider(new providers_1.WorkspaceSymbolProvider(indexAdapter)),
                vscode.languages.registerReferenceProvider(documentSelector, new providers_1.ReferenceProvider(indexAdapter)),
                vscode.languages.registerRenameProvider(documentSelector, new rename_1.RenameProvider(indexAdapter)),
                vscode.languages.registerHoverProvider(documentSelector, new hover_1.HoverProvider(indexAdapter)),
                vscode.languages.registerDocumentLinkProvider(documentSelector, new documentlink_1.DocumentLinkProvider(indexAdapter)),
                vscode.languages.registerFoldingRangeProvider(documentSelector, new folding_1.CodeFoldingProvider(indexAdapter));
            // views
            ...
            command_1.Command.dynamicRegister(plan_1.PlanCommand.CommandName, IndexerNotEnabledCommandHandler);
            command_1.Command.dynamicRegister(commands_1.IndexCommand.CommandName, IndexerNotEnabledCommandHandler);
            command_1.Command.dynamicRegister(validate_1.ValidateCommand.CommandName, IndexerNotEnabledCommandHandler);
            command_1.Command.dynamicRegister(showreferences_1.ShowReferencesCommand.CommandName, IndexerNotEnabledCommandHandler);
            command_1.Command.dynamicRegister(navigatetosection_1.NavigateToSectionCommand.CommandName, IndexerNotEnabledCommandHandler);
            command_1.Command.dynamicRegister(preview_1.PreviewGraphCommand.CommandName, IndexerNotEnabledCommandHandler);
            command_1.Command.dynamicRegister(reindex_1.ReindexCommand.CommandName, IndexerNotEnabledCommandHandler);
        }
