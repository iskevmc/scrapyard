...
class AzureHelpers {
    constructor() {
        this.credentials = utilities.parseCredentialsFile(null);
        this.token = null;
    }
    obtainAuthorizationToken(cb) {
        ...
        let __this = this;
        let postData = "resource=https%3A%2F%2Fmanagement.core.windows.net%2F&client_id=" + azure_client_id + "&grant_type=client_credentials&client_secret=" + azure_secret;
    ...
    getArmTemplateFromResourceGroup(resourceGroup, cb) {
        var http = require('https');
        let __this = this;
        this.obtainAuthorizationToken(function (result) {
            if (result) {
                ...
                let postData = JSON.stringify({ resources: ["*"] });
...
exports.AzureHelpers = AzureHelpers;
